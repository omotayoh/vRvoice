...
# pull the speech server if any of asr/nlp/tts/nmt services are requested
    ...
    if [ "$service_enabled_asr" = true ] || [ "$service_enabled_nlp" = true ] || [ "$service_enabled_tts" = true ] || [ "$service_enabled_nmt" = true ]; then
        ...
        if [[ $riva_target_gpu_family == "tegra" ]]; then
            ...
            docker exec riva-models-download bash -c \
              'cd /usr/local/bin; wget --no-verbose https://api.ngc.nvidia.com/v2/resources/nvidia/ngc-apps/ngc_cli/versions/3.48.0/files/ngccli_arm64.zip; \
            ...
            docker run --init -it --rm --gpus '"'$gpus_to_use'"'  \
              ...
              $image_speech_api download_ngc_models $gmr_speech_models
        fi
...
if [[ $riva_target_gpu_family != "tegra" ]] || ([[ $riva_target_gpu_family == "tegra" ]] && [ -d "$riva_model_loc/rmir" ]); then
    ...
    docker run --init -it --rm --gpus '"'$gpus_to_use'"' \
      ...
      -e "MODEL_DEPLOY_KEY=${MODEL_DEPLOY_KEY}" \
              --name riva-service-maker \
      $image_speech_api deploy_all_models /data/rmir /data/models
      if [ $? -ne 0 ]; then
