...
RIVA_TIMEOUT_SEC=${RIVA_TIMEOUT_SEC:-300}
...
block_until_server_alive() {
    ...
    num_retries=$((RIVA_TIMEOUT_SEC / 10))
...
if [ -n "$ssl_server_key" ] && [ -n "$ssl_server_cert" ]; then
  ...
  if [ "$ssl_use_mutual_auth" = true ]; then
      ssl_args+=" --ssl_use_mutual_auth "
  fi
fi
...
if [ $(docker ps -q -f "name=^/$riva_daemon_speech$" | wc -l) -eq 0 ]; then
    ...
    if [[ $riva_target_gpu_family == "tegra" ]]; then
        ...
        docker_run_args="-p 8000 -p 8001 -p 8002"
    fi
    docker run -d \
        --init \
        --ipc=host \
	...
	-e NMT_REMOTE_ASR_SERVICE=$nmt_remote_asr_service \
	-e NMT_REMOTE_TTS_SERVICE=$nmt_remote_tts_service \
